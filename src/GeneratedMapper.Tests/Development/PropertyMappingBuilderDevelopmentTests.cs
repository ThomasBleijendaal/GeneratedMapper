...
namespace GeneratedMapper.Tests.Development
{
    internal class PropertyMappingBuilderDevelopmentTests
    {
        ...
        [Test]
        public void SimpleMapperAsCollection() => DoTest(true, new PropertyMappingInformation(_mappingInformation).MapFrom("x", false, false).MapTo("x", false, false).AsCollection(DestinationCollectionType.Array, new[] { "x" }, new[] { false }, new[] { "x" }, new[] { false }));

        [Test]
        public void NotNullableToNullableAsCollection() => DoTest(true, new PropertyMappingInformation(_mappingInformation).MapFrom("x", false, false).MapTo("x", true, false).AsCollection(DestinationCollectionType.Array, new[] { "x" }, new[] { false }, new[] { "x" }, new[] { false }));

        [Test]
        public void NullableToNotNullableAsCollection() => DoTest(true, new PropertyMappingInformation(_mappingInformation).MapFrom("x", true, false).MapTo("x", false, false).AsCollection(DestinationCollectionType.Array, new[] { "x" }, new[] { false }, new[] { "x" }, new[] { false }));

        [Test]
        public void NullableToNullableAsCollection() => DoTest(true, new PropertyMappingInformation(_mappingInformation).MapFrom("x", true, false).MapTo("x", true, false).AsCollection(DestinationCollectionType.Array, new[] { "x" }, new[] { false }, new[] { "x" }, new[] { false }));

        [Test]
        public void ValidBaseAsCollectionWithoutMapping() => DoTest(true, _validBase.AsCollection(DestinationCollectionType.Array, new[] { "x" }, new[] { false }, new[] { "x" }, new[] { false }));

        [Test]
        public void ValidBaseAsCollectionUsingMapper() => DoTest(true, _validBase.AsCollection(DestinationCollectionType.Array, new[] { "x" }, new[] { false }, new[] { "x" }, new[] { false }).UsingMapper(_nestedSourceType.Object, _nestedDestinationType.Object).SetMappingInformation(_nestedMappingInformation));

        [Test]
        public void ValidBaseAsCollectionUsingMethod() => DoTest(true, _validBase.AsCollection(DestinationCollectionType.Array, new[] { "x" }, new[] { false }, new[] { "x" }, new[] { false }).UsingMethod("x", "x", Enumerable.Empty<ParameterInformation>()));

        [Test]
        public void ValidBaseAsCollectionUsingResolver() => DoTest(true, _validBase.AsCollection(DestinationCollectionType.Array, new[] { "x" }, new[] { false }, new[] { "x" }, new[] { false }).UsingResolver("x", "x", Enumerable.Empty<ParameterInformation>()));
